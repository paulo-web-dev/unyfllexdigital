...
- Telefone real do remetente: `body.chat.phone` ou `body.message.sender_pn`. **Nunca** `body.message.sender`/`sender_lid` — isso é um LID interno da Uazapi, não telefone. **`chat.phone` NÃO vem limpo (premissa derrubada pelo payload real de 23/07):** veio `"+55 79 8138-5583"` (com `+`, espaço, traço), e num payload de **grupo vem VAZIO** (grupo não tem telefone único). Normalizar sempre por `TelefoneCanonico::normalizar()`; `sender_pn` é um JID (`…@s.whatsapp.net`) e num grupo identifica quem enviou.
- Grupos chegam pela mesma instância (`message.isGroup`/`chat.wa_isGroup` `true`, `chat.wa_chatid` termina em `@g.us`).
...
**O que o primeiro payload real fechou (23/07/2026) — antes era lista de candidatos:**

- **O envelope é PLANO.** A doc não trazia exemplo (`WebhookEvent` = `{event, instance, data}`, `data` livre), então o parser tentava a forma plana e a `data.` (`ProcessarEventoWhatsapp::caminhos()`). O payload real veio plano — `{EventType, chat, instanceName, message, owner, token, …}` no topo, nada sob `data.`. **O lado `data.` foi removido** do parser e do webhook controller. O nome da instância chega como **`instanceName`** (camelCase), não `instance` — o controller passou a incluí-lo (antes caía em `owner` e gravava o telefone na coluna `instance`).
- **A chave de idempotência é `message.id` = `owner:messageid`** (ex. `554195906685:3EB0…`), confirmando este arquivo. `message.messageid` (só o id do provedor) existe em paralelo. A chave do índice único **não muda** — e continua valendo: não trocá-la com linhas já gravadas.
- **`fromMe` / `wasSentByApi` / `source` — valores observados:** mensagem recebida = `fromMe:false`, `wasSentByApi:false`. `source` carrega a plataforma do remetente e **varia**: `"android"` no 1:1, `"ios"` no grupo — não é constante.
...
- **`negociacoes_comercial` — fonte principal do funil comercial** (ligada direto a `classes_id`). Assumiu esse papel quando `leads` foi confirmada vazia (ver abaixo).

  **ATENÇÃO — tabela EM REFORMULAÇÃO, avisado pelo Paulo em 22/07/2026.** Ela vai ser refeita, e por isso a Fatia 4 está pausada nesta fonte (não escrever contra o DDL atual). **Todos os números abaixo descrevem a versão de hoje e não sobrevivem à troca:** tabela refeita não herda medição, e as taxas voltam a ser desconhecidas no dia em que a nova entrar. Rerodar Q3/Q8/Q8c contra ela antes de citar qualquer percentual.

  **Limitação central, medida em 21/07/2026: 2.122 de 3.006 registros (70,6%) têm `whatsapp` inválido** — vazio/nulo **ou** com caractere não-dígito. Sobram **867 aproveitáveis (28,8%)**. A causa é desconhecida — pode ser migração incompleta de um sistema anterior ou preenchimento só em etapa posterior do funil; não presumir uma nem outra. Como não sobra outra tabela de funil ativo do mesmo porte, esses 28,8% são o **teto** de cobertura do matching, não um detalhe de qualidade de dado. E é teto mesmo: **754 dos 867 (87%) estão na forma de 12 dígitos**, ou seja, só casam com a Uazapi pela variante do 9º dígito. **A parcela fixa dessas foi medida em 22/07/2026 e é pequena: dos 628 telefones distintos de 12 dígitos, 589 (93,8%) são deriváveis e 39 são fixos inalcançáveis.**
...
**Consequência prática para quem escrever normalização:** prefixar `55` em número de 10 ou 11 dígitos é o **caminho comum**, não um fallback de exceção. E o caminho completo que a maioria dos registros percorre até casar com a Uazapi é duplo — **prefixar `55` (→ 12 dígitos) e derivar a variante do 9º dígito**. **Correção do payload real (23/07/2026): `chat.phone` NÃO vem sempre com 13** — a premissa "inbound sempre 13, CRM deriva para baixo" estava invertida; os contatos reais chegaram com **12** (o JID de número BR costuma omitir o 9º dígito). Na prática o matching precisa dos **dois sentidos**, e por isso o desenho de 4 candidatas de `IdentificacaoContato` (canônico + variante, com e sem DDI) cobre tanto o inbound de 12 quanto o de 13. Quem tratar qualquer um dos dois passos como caso raro acerta a minoria da base.
