...
- **Colunas novas nascem `varchar(20)`**, não `char(13)`. `users.telefone` é `varchar(14)` — exatamente o tamanho de `+5511987654321`, folga zero: qualquer formatação trunca em silêncio. É o exemplo do que não repetir; 7 bytes de folga custam nada perto de descobrir o truncamento depois.
...
- `contact.telefone` (`int`) e `corporativos.telefone` (`bigint`) são as duas únicas colunas de telefone numéricas do banco e não comportam esse formato: `int` estoura acima de 2147483647, e ambas perdem zero à esquerda e transformam ausência de telefone em `0`. Tratar como não confiáveis para matching até o diagnóstico da Fatia 0 dizer o contrário.

**Comparação — a regra do 9º dígito.** Aceitar dois comprimentos resolve o armazenamento, não o matching: um celular cadastrado com 8 dígitos (12 no canônico) e o mesmo assinante hoje com o 9º dígito (13 — que é o que a Uazapi sempre manda) nunca batem por igualdade de string. Isso troca um erro visível por um **não-match silencioso**. Por isso:
