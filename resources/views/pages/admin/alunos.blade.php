...
<div class="page">
  ...
  <div class="kpi-row">
    <div class="kpi-card">
      <div class="kpi-top"><span class="kpi-label">Total de alunos</span></div>
      <div class="kpi-value">{{ number_format($kpis['totalAlunos'], 0, ',', '.') }}</div>
      <div class="kpi-delta positive">cadastrados</div>
    </div>
    <div class="kpi-card">
      <div class="kpi-top"><span class="kpi-label">Novos hoje</span></div>
      <div class="kpi-value">{{ $kpis['novosHoje'] }}</div>
      <div class="kpi-delta {{ $kpis['novosHoje'] > 0 ? 'positive' : 'neutral' }}">
        {{ $kpis['novosSemana'] }} esta semana
      </div>
    </div>
    <div class="kpi-card">
      <div class="kpi-top"><span class="kpi-label">Com matrícula</span></div>
      <div class="kpi-value">{{ number_format($kpis['totalMatriculas'], 0, ',', '.') }}</div>
      <div class="kpi-delta positive">em minisséries</div>
    </div>
    <div class="kpi-card kpi-gold">
      <div class="kpi-top"><span class="kpi-label">Assistindo</span></div>
      <div class="kpi-value" style="color:var(--gold-400);">{{ number_format($kpis['alunosAtivos'], 0, ',', '.') }}</div>
      <div class="kpi-delta positive">já assistiram algo</div>
    </div>
  </div>
  ...
  <div class="card" style="padding:0;">
    ...
    <form method="GET" action="{{ route('admin.alunos') }}">
      <div class="filter-bar" style="padding:12px 16px;">
        ...
        <button type="submit" class="btn btn-sm" style="font-size:12px;">
          Filtrar
        </button>

        @if($busca || $ordem !== 'recentes')
          <a href="{{ route('admin.alunos') }}" class="btn btn-sm" style="font-size:12px;">✕ Limpar</a>
        @endif

        <div style="flex:1;"></div>
        <span style="font-size:12px;color:var(--fg-4);">
          {{ $alunos->total() }} {{ $alunos->total() === 1 ? 'aluno' : 'alunos' }}
        </span>
      </div>
    </form>
          ...
          @forelse($alunos as $aluno)
            @php
              ...
              $iniciais   = strtoupper(
                  ...
                  (strpos($aluno->name, ' ') !== false
                      ? substr(strrchr($aluno->name, ' '), 1, 1)
                      : '')
              );
            @endphp
            <tr>
              ...
              <td>
                <div style="display:flex;align-items:center;gap:10px;">
                  <div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,#00C2FF,#0072FF);color:#061224;font-size:11px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                    {{ $iniciais }}
                  </div>
                  <div style="min-width:0;">
                    <div style="font-size:13px;font-weight:500;color:var(--fg-1);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:180px;">
                      {{ $aluno->name }}
                    </div>
                    <div style="font-size:11px;color:var(--fg-4);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:180px;">
                      {{ $aluno->email }}
                    </div>
                  </div>
                </div>
              ...
              <td>
                ...
                @if($aluno->setor)
                  <div style="font-size:11px;color:var(--fg-4);">{{ $aluno->setor }}</div>
                @endif
              </td>
              ...
              <td>
                @if($matriculas > 0)
                  <span style="display:inline-flex;align-items:center;gap:4px;padding:3px 8px;border-radius:999px;background:rgba(0,163,255,0.10);color:var(--brand-300);font-size:11px;font-weight:600;border:1px solid rgba(0,163,255,0.25);">
              ...
              <td>
                <div style="display:flex;align-items:center;gap:8px;">
                  <span style="font-family:var(--font-mono);font-size:13px;color:{{ $capsulas > 0 ? 'var(--fg-1)' : 'var(--fg-4)' }};">
                    {{ $capsulas > 0 ? number_format($capsulas, 0, ',', '.') : '—' }}
                  </span>
                  @if($capsulas > 0)
              ...
              <td>
                <span style="
                  ...
                  background:{{ ($aluno->power ?? 0) > 10 ? 'rgba(232,183,101,0.12)' : 'rgba(255,255,255,0.05)' }};
                  color:{{ ($aluno->power ?? 0) > 10 ? 'var(--gold-400)' : 'var(--fg-4)' }};
                  border:1px solid {{ ($aluno->power ?? 0) > 10 ? 'rgba(232,183,101,0.25)' : 'var(--line-1)' }};
                ">
                  {{ $aluno->power ?? 0 }}
                </span>
              </td>
          ...
          @empty
            <tr>
              <td colspan="7" style="text-align:center;color:var(--fg-4);padding:48px;font-size:13px;">
                @if($busca)
                  Nenhum aluno encontrado para "<strong>{{ $busca }}</strong>".
                @else
    ...
    <div style="padding:12px 16px;display:flex;justify-content:space-between;align-items:center;border-top:1px solid var(--line-2);font-size:12px;color:var(--fg-4);">
      <span>
        @if($alunos->total() > 0)
          {{ $alunos->firstItem() }}–{{ $alunos->lastItem() }} de {{ number_format($alunos->total(), 0, ',', '.') }}
        @endif
      </span>
      <div>
        {{ $alunos->links() }}
...
@push('styles')
<style>
/* Paginação do Laravel com visual do painel */
...
nav[role="navigation"] [aria-current] {
  background:rgba(0,163,255,0.15); border-color:rgba(0,163,255,0.4);
  color:var(--brand-200);
}
</style>
@endpush
